...
int merge_lists(struct node* p_list_1, struct node* p_list_2, struct node** pp_merged_list)
{
    ...
    while(1)
    {
        if(p_run_1->next == NULL)
        {
            while(p_run_2->next != NULL)
            {
        ...
        if(p_run_2->next == NULL)
        {
            while(p_run_1->next != NULL)
            {
...
void test_suite_3(void)
{
    ...
    for(i = 0; i < 5; ++i)
    {
        ...
        assert(ret == SUCCESS);
        ret = insert_at_end(p_list_2, (i+1) * 100);
        assert(ret == SUCCESS);
    }
    ...
    for(i = 0; i < 5; ++i)
    {
        ret = insert_at_end(p_list_1, rand());
        assert(ret == SUCCESS);
        ret = insert_at_end(p_list_2, rand());
        assert(ret == SUCCESS);
    }

    show_list(p_list_1, "Showing Original list 1 before Merging the Lists:");
    show_list(p_list_2, "Showing Original list 2 before Merging the Lists:");

    ret = merge_lists(p_list_1, p_list_2, &p_merged_list);
    assert(ret == SUCCESS);
    show_list(p_list_1, "Showing Merged List:");
